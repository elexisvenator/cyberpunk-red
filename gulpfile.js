 ...
 */
function buildDevBundle(cb) {
	webpack(webpackDevConfig).run(runWebpack(cb));
}

function buildProdBundle(cb) {
	webpack(webpackProdConfig).run(runWebpack(cb));
}
...
function runWebpack(cb) {
	return function(err, stats) {
		if (err) {
			...
			console.log('Webpack: ', chalk.blueBright(`finished build`));
			cb();
		}
	};
}
...
async function linkUserData() {
	...
	try {
		...
		if (path.resolve(config.dataPath)) {
			...
			linkDir = path.join(path.resolve(config.dataPath), 'Data', destDir, name);
		} else {
		...
		if (argv.clean || argv.c) {
			...
			console.log(
				chalk.green(
					`Copying build from ${chalk.blueBright(
						path.resolve('./dist')
					)} to ${chalk.blueBright(linkDir)}`
				)
			);
			await fs.symlink(path.resolve('./dist'), linkDir, 'dir');
		}
		return Promise.resolve();
	} catch (err) {
		Promise.reject(err);
